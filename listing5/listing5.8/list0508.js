with(document) {
  let A = [5, 3, 2];
  write("A = [" + A + "]<br>");
  write("Результат выполнение команды <code>A.reduce(Math.pow)</code>:<br>");
  write(A.reduce(Math.pow) + "<br>");
  write("Результат выполнение команды <code>A.reduceRight(Math.pow)</code>:<br>");
  write(A.reduceRight(Math.pow) + "<br>");
  let B = [1, 2, , 4, 5, 6, 7, , 9];
  write("B = [" + B + "]<br>");
  write("После выполнения команды <code>B.ofrEach(fillIt)</code>:<br>");
  B.forEach(fillIt);
  write("B=[" + B + "]<br>");
  B.splice(2, 1);
  ...
  write("Результат выполнения <code>B.every(isInRange)</code>:<br>");
